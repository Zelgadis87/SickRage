            ...
            % if sickbeard.HOME_LAYOUT == 'poster':
                ...
                <span class="pull-right show-option"> ${_('Sort By')}:
                    <select id="postersort" class="form-control form-control-inline input-sm" title="Poster Sort">
						...
						<option value="added" data-sort="${srRoot}/setPosterSortBy/?sort=added" ${('', 'selected="selected"')[sickbeard.POSTER_SORTBY == 'added']}>${_('Added Date')}</option>
                        <option value="lastseen" data-sort="${srRoot}/setPosterSortBy/?sort=lastseen" ${('', 'selected="selected"')[sickbeard.POSTER_SORTBY == 'lastseen']}>${_('Last Seen')}</option>
